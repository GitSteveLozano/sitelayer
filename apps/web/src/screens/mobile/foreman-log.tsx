...
import { DEyebrow, DH1, DKpi, DKpiStrip } from '../../components/d/index.js'
...
/**
 * Responsive Foreman Daily Log. Mounts the dense desktop split (agent draft +
 * notes editor on the left, sticky photo grid + Submit on the right; the former
 * `screens/desktop/fm-log.tsx`, `ForemanLogDesktop`) at >=1024px and the mobile
 * stacked composer below it. Both share the SAME daily-log hook surface
 * (find-or-create today's draft, voice-to-log agent, auto-assembly prefill,
 * OCC-versioned patch/submit); only the layout differs. Only ONE render mounts
 * at a time, so the find-or-create effect doesn't double-fire across surfaces.
 */
export function ForemanLog(props: { bootstrap: BootstrapResponse | null; companySlug: string }) {
  const isDesktop = useIsDesktop()
  return isDesktop ? <ForemanLogDesktop bootstrap={props.bootstrap} /> : <ForemanLogMobile {...props} />
}

/** Desktop-route alias — kept so desktop-workspace.tsx can keep importing
 *  `FmLog` after the desktop twin file was deleted. */
export const FmLog = ForemanLogDesktop

function ForemanLogMobile({ bootstrap }: { bootstrap: BootstrapResponse | null; companySlug: string }) {
      ...
      <MBody pad>
        {projects.length === 0 ? (
          ...
          <>
            ...
            {log ? (
              <DailyLogEditor log={log} bootstrap={bootstrap} onDone={() => navigate(nav.today)} />
            ) : (
...
// ---------------------------------------------------------------------------
// Desktop composition — the former screens/desktop/fm-log.tsx, folded in
// verbatim (data + behavior preserved). Helpers are suffixed `Desktop` to avoid
// colliding with the mobile composer's same-named locals.
// ---------------------------------------------------------------------------
 ...
 */
function parseWeatherDesktop(raw: unknown): { value: string; meta: string | null } | null {
  if (!raw) return null
...
function ForemanLogDesktop({ bootstrap }: { bootstrap: BootstrapResponse | null }) {
          ...
          <>
            {projects.length > 1 ? (
              <div style={{ maxWidth: 360 }}>
                <div style={MONO_LABEL_DESKTOP}>Project</div>
                <MSelect
                  value={projectId}
                  onChange={(e) => setProjectId(e.currentTarget.value)}
            ...
            {!projectId ? (
              ...
              <DailyLogEditorDesktop key={log.id} log={log} bootstrap={bootstrap} />
...
interface DailyLogEditorDesktopProps {
  log: DailyLog
  bootstrap: BootstrapResponse | null
}

function DailyLogEditorDesktop({ log, bootstrap }: DailyLogEditorDesktopProps) {
  ...
  // only on a still-empty draft. Mirrors the mobile composer exactly.
  useEffect(() => {
  ...
  const weather = parseWeatherDesktop(log.weather)
    ...
    <>
      ...
      <div className="d-split">
        ...
        <div className="d-stack" style={{ gap: 20 }}>
          <div>
            <div style={MONO_LABEL_DESKTOP}>Agent draft</div>
            <div style={{ marginTop: 8 }}>
              <VoiceToLogBlockDesktop
                dailyLogId={log.id}
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={MONO_LABEL_DESKTOP}>Notes</div>
        ...
        <div className="d-card" style={{ position: 'sticky', top: 24 }}>
          <div style={MONO_LABEL_DESKTOP}>Photos · {photoCount}</div>
          <div style={{ marginTop: 12 }}>
            <PhotoGridDesktop log={log} />
          </div>
...
/**
 * Photo grid (desktop). Reads per-photo metadata from `useDailyLogPhotos`,
 * falling back to the legacy `log.photo_keys` array before the query resolves
 * so the grid never flashes empty for freshly-uploaded photos.
 */
function PhotoGridDesktop({ log }: { log: DailyLog }) {
...
interface VoiceToLogBlockDesktopProps {
  dailyLogId: string
  isSubmitted: boolean
...
function VoiceToLogBlockDesktop({
  dailyLogId,
  isSubmitted,
  attributionCounts,
  onApplyProposal,
}: VoiceToLogBlockDesktopProps) {
