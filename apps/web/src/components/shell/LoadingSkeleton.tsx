...
export interface SkeletonProps {
  ...
  w?: string
  /** Override the default square corners. */
  rounded?: string
}

export function Skeleton({ className, h = 'h-3', w = 'w-full', rounded = 'rounded-none' }: SkeletonProps) {
  ...
  return (
    <div role="status" aria-label="Loading" aria-busy="true" className={cn('px-4 space-y-2', className)}>
      {Array.from({ length: count }, (_, i) => (
        <div key={i} className="bg-card border-2 border-ink px-3.5 py-3 flex items-center gap-3">
          <span aria-hidden="true" className="block w-8 h-8 bg-card-soft animate-pulse" />
