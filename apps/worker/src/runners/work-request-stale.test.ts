...
describe('createWorkRequestStaleRunner', () => {
  ...
  it('posts work_item_obstructed observation events to mesh on transition', async () => {
    ...
    try {
      ...
      // projectkit stamps the current contract version (1.4.0 since v0.9.0 —
      // addressed dispatch: Concern.audience/assignee/acceptance).
      expect(envelope.contract_version).toBe('1.4.0')
