 ...
 * "generate next billing run" path that materializes a `rental_billing_run`
 ...
 * in `rental-billing-state.ts`. The contract-line CRUD (lines, rate tiers)
 * lives in `rental-contract-lines.ts`.
 *
 * Routes:
 ...
 */
export async function handleRentalContractsRoutes(
  ...
  if (req.method === 'PATCH' && url.pathname.match(/^\/api\/rental-contracts\/[^/]+$/)) {
    ...
    return patchVersionedEntity({
      ...
      body,
      entityType: 'job_rental_contract',
      entityName: 'rental contract',
      table: 'job_rental_contracts',
      id: contractId,
