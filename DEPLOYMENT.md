...
**Queue processing.** The API and worker use Postgres-backed queue tables instead of an external Redis dependency. `mutation_outbox` and `sync_events` are claimed with a short processing lease, using `FOR UPDATE SKIP LOCKED`, then marked `applied`. This is still a local/simulated processor until live QBO sync is enabled, but it prevents multiple workers from processing the same row and leaves retry metadata (`attempt_count`, `next_attempt_at`, `error`) for the real connector.
