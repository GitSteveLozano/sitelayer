...
**Routing topology (read before adding a reachable route).** The canonical runtime shell is `apps/web/src/screens/mobile-shell.tsx` (`MobileShell`), mounted directly at `App.tsx`'s `/*` route (the former `routes/workspace.tsx` wrapper was removed — `App.tsx` now mounts `MobileShell` itself). `MobileShell` carries its OWN inline `<Routes>` table, including the `projects/:projectId/*` and `rentals/*` catchalls. To add a new reachable mobile route, add it inside **`mobile-shell.tsx` before those catchalls** (otherwise a catchall swallows it); a full-screen/specialized route instead mounts directly in `App.tsx` (e.g. `/financial/*`, `/more`) or in `more.tsx` / `financial.tsx`. **The legacy dead route files `routes/{projects,rentals,schedule,home,time,log,crew}.tsx` were removed (2ceeab3d); every surviving `routes/*.tsx` is imported and live. Do NOT recreate per-feature route files there — routes go in `mobile-shell.tsx` / `App.tsx` as above.**
...
| Managed Postgres                 | `sitelayer-db`, ID `9948c96b-b6b6-45ad-adf7-d20e4c206c66`, Postgres 18, `db-s-1vcpu-2gb`, single node (`NumNodes=1`), Toronto `tor1`, online. NO PITR — PITR needs a standby node (`--num-nodes 2`), NOT more RAM; real RPO ≈ 24 h via daily logical pg_dump (see docs/DR_RESTORE.md + BACKUP_STRATEGY.md)                                                                                                                                                                                                             |
...
| Backups                          | DO managed Postgres automatic backups exist (coarse, ~daily, NOT point-in-time); logical Postgres backup, Postgres off-host copy, off-region Spaces copy (`scripts/backup-to-offregion.sh`, daily 06:00 UTC), blueprint-volume fallback copy, restore-drill, and timer-monitor timers are active. Full topology + restore-drill cadence in `docs/BACKUP_STRATEGY.md`                                                                                                                                                   |
...
| **Monorepo**        | npm workspaces                                             | apps: api, web, worker; packages: config, domain, logger, queue, workflows, projectkit-bridge, formula-evaluator, scenario, capture-schema, capture-catalog, pipe-blueprint, pipe-roomplan, pipe-drone, pipe-photogrammetry                                        |
...
│   ├── pipe-blueprint/      # Blueprint PDF → quantities; prod default = Gemini (BLUEPRINT_VISION_MODE=gemini, async via worker); Anthropic/Claude = opt-in alt (BLUEPRINT_VISION_MODE=live). See "Blueprint storage hygiene" rule #4.
...
- **Routing**: `server.ts` hands each request to `apps/api/src/routes/dispatch.ts`, which dispatches to the ~108 per-feature handler modules in `apps/api/src/routes/` (~150 endpoints). CORS handling for frontend/worker origins.
...
**Endpoints** (partial sample — the canonical registry is `apps/api/src/routes/dispatch.ts` plus the ~108 handler modules in `apps/api/src/routes/` (~150 endpoints total). `server.ts` is HTTP + auth + middleware only; it does not enumerate routes. Don't treat the list below as exhaustive):
...
Each deterministic workflow has a registered reducer in `packages/workflows/src/` (see `docs/DETERMINISTIC_WORKFLOWS.md` for the design rules). **The authoritative list of registered workflows is `registry.ts` / `listWorkflows()` — 20 as of 2026-06-13** (the "Currently registered" line in `docs/DETERMINISTIC_WORKFLOWS.md` is guarded against drift by a consistency test, `packages/workflows/src/registry-docs.test.ts`). The table below is an illustrative at-a-glance summary, not exhaustive — workflows not listed here (e.g. `change_order`, `estimate_share`, `asset_deployment`, `tenant_provision`) are still registered.
