...
export function ProjectDetail({ projectId, onBack }) {
  const { project: p, entries, loading } = useProject(projectId)
    ...
    <div style={{ padding: '32px 36px', maxWidth: 980 }}>
      ...
      {!hasD && (
        <Card style={{ marginBottom: 14, textAlign: 'center', padding: 32 }}>
          <div style={{ fontSize: 14, color: TH.muted }}>No time entries yet. Log crew hours to see live performance metrics.</div>
        </Card>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {/* Labor by item */}
        <Card>
          <Label>Labor by Service Item</Label>
          {m.byItem?.length === 0 ? (
            ...
            m.byItem?.map(row => (
              <div key={row.item} style={{ paddingBottom: 10, marginBottom: 10, borderBottom: `1px solid ${TH.border}` }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: TH.muted }}>
                  ...
                  <span>{row.rate.toFixed(2)} sqft/hr</span>
                </div>
              </div>
            ))
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {(p.target_sqft_per_hr || 0) > 0 && (
            <Card>
              <Label>Crew Performance — Bonus Tracker</Label>
