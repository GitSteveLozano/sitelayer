...
- **What is genuinely NOT built:** a scaffold _designer_ (scaffold today is catalog+BOM+approval, not design). NOTE: captured geometry **is** piped into the 3D renderer (since `05be647d`, `apps/web/src/lib/takeoff/captured-geometry-3d.ts`), and RoomPlan **wall** geometry now renders too (since `2398be1f`) — it is no longer manual-polygon-only. Deeper detail: `docs/BLUEPRINT_TO_3D_PREVIEW.md`, `docs/MULTI_DRAFT_TAKEOFF_SPEC.md`.
...
The runtime shell is `apps/web/src/screens/mobile-shell.tsx` (`MobileShell`), mounted directly at `App.tsx`'s `/*` (the former `routes/workspace.tsx` wrapper was removed), with its own inline route table including the `projects/:projectId/*` and `rentals/*` catchalls. Add a new reachable mobile route inside `mobile-shell.tsx` **before** those catchalls, or mount a full-screen route directly in `App.tsx`/`more.tsx`/`financial.tsx`. The standalone `routes/{projects,rentals,schedule,home,time,log,crew}.tsx` modules were removed (2ceeab3d) — every surviving `routes/*.tsx` is imported and live, so do not recreate per-feature route files there.
