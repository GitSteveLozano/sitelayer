...
- **Multi-draft system — Shipped 2026-05-10** — Multiple measurement drafts per project. Create, rename, archive, duplicate. Each draft owns its own measurements and estimate; bid-vs-scope compares per draft. Capture pipelines (manual, blueprint vision, photogrammetry, drone) attach to drafts and can promote captured `TakeoffResult` rows into committed measurements. Landed across PRs #266–#270 (Phase A: schema, CRUD, picker UI, per-draft estimates, NOT NULL lock), #272–#274 (Phase C: capture artifacts schema, capture-to-draft endpoint, Capture-from UI), #287 (live `blueprint_vision` mode — Anthropic SDK at the time; the **prod default is now Gemini, run async in the worker** — see CLAUDE.md "Blueprint storage hygiene" rule #4), #289 (promote captured result to measurements). See `docs/MULTI_DRAFT_TAKEOFF_SPEC.md` for the schema/API/UI contract.
...
packages/
  ...
  workflows/ — Deterministic temporal-style workflow reducers (20 registered: rental-billing, estimate-push, project-lifecycle, time-review, … — see docs/DETERMINISTIC_WORKFLOWS.md + ADR 0006)

docker/
  postgres/init/  — SQL migration files (000_baseline.sql floor + additive 0NN_*.sql)
```
