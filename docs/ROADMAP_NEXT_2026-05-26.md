...
> **STALE (2026-06-12):** This roadmap is a 2026-05-26 snapshot and has drifted
> from reality. Use [`CRITICAL_PATH.md`](../CRITICAL_PATH.md) and the
> 2026-06-12 audit (debt campaign) as the live source of truth. Known dead
> entries: the Track E "ORM migration" row below contradicts the LOCKED
> no-ORM decision in root `CLAUDE.md` (raw parameterized SQL stays; at most a
> SQL-first type generator like PgTyped) — do not execute it. Capture
> pipelines, deploy model (`scripts/deploy.sh`, zero GitHub Actions), and the
> takeoff canvas have all moved since this was written.
...
| **P2** | ~~**ORM migration (Prisma/Drizzle)**~~ ❌ **WITHDRAWN (2026-06-12)** | ~~CLAUDE.md flags raw-SQL-in-handlers as "unsustainable beyond ~100 queries"; already past threshold. Post-pilot.~~ **Contradicts the locked no-ORM decision** (root `CLAUDE.md` → "Database ORM / Query Layer": raw parameterized SQL stays — ORMs fight `FOR UPDATE SKIP LOCKED`, the RLS GUC, and the immutable-migration discipline). Do not relitigate; if type-safety pain is ever measured, evaluate a SQL-first generator (PgTyped) only. | —        |
