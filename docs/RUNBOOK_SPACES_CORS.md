...
1. Add `BLUEPRINT_DOWNLOAD_PRESIGNED=1` to the production environment.
   ...
   (the deploy reuses it; the `ops/env/production.env.json` manifest defines
   names/scope). Deploys are local-fleet — there is no GitHub Actions
   `production` environment.

   ```bash
   ssh sitelayer@165.245.230.3 \
     "grep -q '^BLUEPRINT_DOWNLOAD_PRESIGNED=' /app/sitelayer/.env \
        && sed -i 's/^BLUEPRINT_DOWNLOAD_PRESIGNED=.*/BLUEPRINT_DOWNLOAD_PRESIGNED=1/' /app/sitelayer/.env \
        || printf '\nBLUEPRINT_DOWNLOAD_PRESIGNED=1\n' >> /app/sitelayer/.env"
   ```

2. Bounce the API container so it re-reads the env:
