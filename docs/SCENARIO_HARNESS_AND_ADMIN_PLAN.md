...
>
> - **v1.0 — 2026-06-01. COMPLETE — P0–P5 + impersonation banner + scenario apply.** Everything shipped & verified on `dev-np`. P3 now includes the **Apply / spin-up-demo** mutation: `POST /api/admin/scenarios/:slug/apply` (gated, prod-blocked, optional company retarget) on `applyScenarioFixture` + `makeScenarioApplyRunner`, with an Apply button in the console — proven against an ephemeral Postgres (seeds + retargets a fixture to a fresh company; 4 golden tests). Commits `207ce72` (apply backend) `b439be3` (apply UI). Whole initiative typechecks/builds/tests green (`apps/api` + `apps/web` + `packages/scenario`). **Remaining follow-on (not a blocker):** the guided `nextEvents` timeline EDITOR for interactive in-browser scenario authoring — read / preview / apply all shipped; step-by-step authoring is the next slice.
...
- **P3 — `/admin` UI shell + scenario builder**: ✅ **DONE (2026-06-01).** Read endpoints `/api/admin/workflows` + `/api/admin/scenarios[/:slug/plan]` (on the P0 engine + registry); a lazy, bundle-safe Site Admin console at `/admin` (Companies / Workflows / Scenarios with a live apply-plan preview). **Apply / spin-up-demo shipped** (`POST /api/admin/scenarios/:slug/apply` + console button). _Follow-on: the guided `nextEvents` timeline EDITOR for interactive authoring — read / preview / apply all shipped, step-by-step authoring is next._
