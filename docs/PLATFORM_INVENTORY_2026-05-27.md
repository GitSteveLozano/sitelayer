...
2. **Captured 3D geometry → renderer** — _shipped (2026-05-27, relative scale)._ `geometry-3d.ts` renders promoted `kind:'capture'` polygons by bounds-normalizing the capture set to a relative scale and bypassing the blueprint/page filter; selecting the capture draft in the 3D preview shows them. **Drone** captures render (lat/lon polygons), and **blueprint** captures now render too — `pipe-blueprint` emits `geometry.surfaces[].polygon` (image pixels) + quantity `geometryRefs`, which promote attaches to the measurement. Remaining follow-up: absolute per-source calibration (pixels→ft via `provenance.scaleFt`, lat/lon→ft projection) so captures show true scale instead of relative.
