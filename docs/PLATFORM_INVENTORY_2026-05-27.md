...
2. ~~Captured 3D geometry → renderer~~ — **shipped (2026-05-27).** `geometry-3d.ts` renders promoted `kind:'capture'` polygons and bypasses the blueprint/page filter; selecting the capture draft in the 3D preview shows them. **Blueprint** captures render at **true scale** — `pipe-blueprint` emits `geometry.surfaces[].polygon` (image pixels) + `geometryRefs`; promote threads `provenance.scaleFt` (pixels-per-foot) onto the measurement, and the renderer converts pixels→feet. **Drone** captures (lat/lon, no pixel ratio) render bounds-normalized at relative scale. Remaining nicety: lat/lon→ft projection for true-scale drone footprints.
