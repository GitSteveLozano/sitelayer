...
1. **Scaffold designer** (3D design → auto-BOM → load/bracing) — _in progress; core + 3D UI shipped 2026-05-27._ `packages/domain/src/scaffold-design.ts` `generateScaffoldModel(spec)` is the pure, deterministic core (bays×lifts → 3D members + part demand by role+length). A live designer screen is wired at `/scaffold-designer` (reachable from the scaffold catalog): edit the spec, see the scaffold in three.js (`scaffold-3d-scene.tsx` via the tested `lib/scaffold/scaffold-scene.ts` mapper), and read the auto-BOM (part demand) table. The auto-BOM persists: `resolveScaffoldBom` (domain) matches part demand → `catalog_parts` by the `attrs.role` convention + nearest `length_mm`, and `POST /api/projects/:id/scaffold-designs/bom` runs the generator, resolves, and writes a draft `boms`/`bom_lines` (`source='scaffold_design'`) with weight rollup, reporting any unresolved roles. The designer screen closes the loop: a "Save as BOM" panel (project + scaffold-system picker) POSTs to the endpoint and reports the saved draft BOM + any unresolved roles. Remaining slices: load/bracing checks; a richer member-level render (decks already render as planes); BOM approve→inventory rollup is already wired via the existing `scaffold_ops_approval` workflow.
