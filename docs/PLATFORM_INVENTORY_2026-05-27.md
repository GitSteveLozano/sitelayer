...
2. **Captured 3D geometry → renderer** — _partly shipped (2026-05-27, relative scale)._ `geometry-3d.ts` now renders promoted `kind:'capture'` polygons by bounds-normalizing the capture set to a relative scale and bypassing the blueprint/page filter; selecting the capture draft in the 3D preview shows them. **Works for drone** (emits `geometry.surfaces[].polygon` in lat/lon). **Blueprint captures don't render yet** — `pipe-blueprint`'s `TakeoffResult` ships only `quantities` + a pixel-space artifact, with no `geometry` field, so promote never gets a polygon. Follow-ups: populate `geometry.surfaces[].polygon` + quantity `geometryRefs` in `pipe-blueprint` (makes blueprint captures render); then absolute per-source calibration (pixels→ft via `provenance.scaleFt`, lat/lon→ft projection).
