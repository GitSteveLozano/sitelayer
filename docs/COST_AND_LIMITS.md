...
| Preview droplet (`sitelayer-preview`)   | `s-2vcpu-4gb` Toronto               | Hosts per-PR preview + dev/demo stacks + off-host backups (no GHA runner — repo runs zero GitHub Actions)                                                                                  | $24.00      |
| Managed Postgres (`sitelayer-db`)       | `db-s-1vcpu-2gb` Toronto, 1 node    | PG 18; coarse daily automatic backups only — NO PITR (single node, no standby; PITR needs a standby node, NOT more RAM). Real RPO ≈ 24 h via daily logical pg_dump; see docs/DR_RESTORE.md | $15.00      |
...
| Postgres connections                         | ~47 (`db-s-1vcpu-2gb` tier)                                                                      | > 38 used                                                                          | Resize up (`db-s-2vcpu-4gb` ≈ 97 connections)                                                                                                                                                                           | +$15                   |
| Postgres PITR (none today)                   | No PITR — single-node `db-s-1vcpu-2gb`, no standby (PITR is gated on a standby node, NOT on RAM) | Pilot data RPO must drop below the ≈24 h daily-dump window                         | Add a standby node (`doctl databases resize 9948c96b-... --num-nodes 2`); the cluster is already `db-s-1vcpu-2gb`, so this is an HA upgrade, not a RAM resize. DO then enables continuous PITR — see docs/DR_RESTORE.md | +$15+ (standby node)   |
