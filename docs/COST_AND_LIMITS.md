...
| Managed Postgres (`sitelayer-db`)       | `db-s-1vcpu-1gb` Toronto, 1 node    | PG 18; coarse daily automatic backups only — NO PITR (single node, no standby). Real RPO ≈ 24 h via daily logical pg_dump; see docs/DR_RESTORE.md | $15.00      |
...
| Postgres PITR (none today)                   | No PITR — single-node `db-s-1vcpu-1gb`, no standby | Pilot data RPO must drop below the ≈24 h daily-dump window                         | Resize to a standby-capable plan (e.g. `db-s-1vcpu-2gb` with a standby node); DO then enables continuous PITR — see docs/DR_RESTORE.md | +$15+ (standby node)   |
