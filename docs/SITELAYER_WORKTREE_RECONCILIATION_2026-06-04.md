...
- Steve/public collaborator access is now partially implemented through a dedicated feedback-invite token surface, not `capture_invite` as auth:
  ...
  - STT deployment plumbing now exposes the capture-analysis and callback-reconciler env knobs. `host.docker.internal:5678` is same-host only; dev/prod audio STT now relies on the local GPU media worker unless Whisper is explicitly provisioned on the droplet.
