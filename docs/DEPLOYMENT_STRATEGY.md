...
"The promotion model", `docs/RELEASE_GATES.md`, and the promotion model in
[`docs/ENVIRONMENTS_AND_MIGRATIONS.md`](./ENVIRONMENTS_AND_MIGRATIONS.md) §5). The
demo→`main` track lands **dormant** while `main == dev`: until the first real
`dev → main` promotion the two refs are identical, so demo fast-following `main`
deploys exactly what tracking `dev` would, and the split only takes effect once
promotion creates daylight between them. PR previews isolate per-slug via
`PGOPTIONS` search_path and self-reap on PR close. The single preview droplet
(2 vCPU/4 GB, Traefik on one router) hosts **dev + demo + every PR preview** and is
also the off-host backup target (`10.118.0.2`).
